...
class DCNN_MLP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, out_channels_cnn2, out_channels_cnn3,
        ...
        self.fc1 = nn.Linear(out_channels_cnn6*kernel_size + out_features , nlabels)
    ...
    def forward(self, x, xref, f):
        ...
        # THIRD PASS THE REFERENCE SPECTROGRAM THROUGH THE SAME CNN LAYERS
        ...
        xref = xref.reshape([xref.shape[0], 3, xref.shape[1], xref.shape[2]])
...
x_ws1, xref = spectrogram(X_ws1, xref_ws1)
...
xr = TensorDataset(torch.Tensor(xref))
...
if args.train:
    print("Training...")
    my_DCNN.trainloop(trainloader, validloader, xrefloader)
    torch.save(my_DCNN.state_dict(), path + 'checkpoint_SERVER_static_2dcnn_potencia2_6_layers_xreference_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.pth')

# EVALUATION METRICS FOR WS1 COMPLETE DATASET
else:
    # accuracy
    my_DCNN.load_state_dict(torch.load(path + 'checkpoint_EarlyStopping_bestvalepoch_SERVER_static_2dcnn_potencia2_6_layers_xreference_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.pth'))

    acc_train = my_DCNN.eval_performance(trainloader, xrefloader)
    acc_val = my_DCNN.eval_performance(validloader, xrefloader)
    acc_tst = my_DCNN.eval_performance(testloader, xrefloader)
    ...
    # AUC 
    auc_train = my_DCNN.get_auc(trainloader, xrefloader, y_tr, name='ROC_2DCNN_TRAIN')
    auc_val = my_DCNN.get_auc(validloader, xrefloader, y_val, name='ROC_2DCNN_VALID')
    auc_test = my_DCNN.get_auc(testloader, xrefloader, y_test, name='ROC_2DCNN_LSTM_TRAIN')
    ...
    for conf in grid:
        ...
        x_ws1, x_ref_ws1 = spectrogram(X_ws1, xref_ws1)
        x_ws2, x_ref_ws2 = spectrogram(X_ws2, xref_ws2)
        x_ws3, x_ref_ws3 = spectrogram(X_ws3, xref_ws3)
        x_ws4, x_ref_ws4 = spectrogram(X_ws4, xref_ws4)
        ...
        xr_ws1 = TensorDataset(torch.Tensor(x_ref_ws1))
        xrefloader_ws1 = DataLoader(xr_ws1, batch_size=4, shuffle=True)

        acc_ws1 = model.eval_performance(dataloader_ws1, xrefloader_ws1)
        auc_ws1 = model.get_auc(dataloader_ws1, xrefloader_ws1, y_bin_ws1, name= 'ROC_2DCNN_WS1_'+str(list(conf.items())))
        ...
        xr_ws2 = TensorDataset(torch.Tensor(x_ref_ws2))
        xrefloader_ws2 = DataLoader(xr_ws2, batch_size=4, shuffle=True)
        
        acc_ws2 = model.eval_performance(dataloader_ws2, xrefloader_ws2)
        auc_ws2 = model.get_auc(dataloader_ws2, xrefloader_ws2, y_bin_ws2, name= 'ROC_2DCNN_WS2_'+str(list(conf.items())) )
        ...
        xr_ws3 = TensorDataset(torch.Tensor(x_ref_ws3))
        xrefloader_ws3 = DataLoader(xr_ws3, batch_size=4, shuffle=True)
        
        acc_ws3 = model.eval_performance(dataloader_ws3, xrefloader_ws3)
        auc_ws3 = model.get_auc(dataloader_ws3, xrefloader_ws3, y_bin_ws3, name= 'ROC_2DCNN_WS3_'+str(list(conf.items())))
        ...
        xr_ws4 = TensorDataset(torch.Tensor(x_ref_ws4))
        xrefloader_ws4 = DataLoader(xr_ws4, batch_size=4, shuffle=True)
        
        acc_ws4 = model.eval_performance(dataloader_ws4, xrefloader_ws4)
        auc_ws4 = model.get_auc(dataloader_ws4, xrefloader_ws4, y_bin_ws4, name= 'ROC_2DCNN_WS4_'+str(list(conf.items())))
    ...
    results.to_csv(path + 'results_SERVER_2DCNN_COMPLETE_MODEL_MULTICLASS_xref_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.csv')
