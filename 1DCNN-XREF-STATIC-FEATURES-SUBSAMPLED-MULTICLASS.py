# -*- coding: utf-8 -*-
"""
Created on Thu Mar 31 12:07:34 2022
...
def plot_roc_curve(fper, tper, auc, title='', name=''):
    plt.figure()
...
#  DESIGN CNN: 6 LAYERS, BINARY CLASSIFICATION, XREF, MLP STATIC FEATURES 
class CNN_MLP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, out_channels_cnn2, out_channels_cnn3,
        ...
        self.final_dim_cnn = (int(out_channels_cnn6 * (int(sequence_length/kernel_maxpool**6))))+ out_features + (int(out_channels_cnn6 * int(int(sequence_length/kernel_maxpool**6)*batch_size_ref))) # AVERIGUAR COMO SACAR ESTO
        ...
        self.fc1 = nn.Linear((int(out_channels_cnn6 * (int(sequence_length/kernel_maxpool**6))))*2+ out_features , nlabels)
    ...
    def forward(self, x, xref, f, sigma=1, h0=None, valid=False):
        ...
        # FIRST PASS THE INPUT SIGNAL THROUGH 2 CNN
        #print('input signal shape', x.shape)
        x = x.reshape((batch_size,1,seq_length))
        #print('INPUT SIGNAL SHAPE ', x.shape)
        #print(type(x))
        x = self.conv1(x)
        #print('OUTPUT OF CONV1 LAYER SIGNAL SHAPE ', x.shape)
        x = self.relu(x)
        ...
        x = self.conv2(x)
        #print('OUTPUT OF CONV2 LAYER SIGNAL SHAPE ', x.shape)
        x = self.relu(x)
        ...
        x = self.relu(x)
        x = self.pool(x)
        #print('OUTPUT OF CONV6 LAYER SIGNAL SHAPE ', x.shape)
        
        # ADD REGULARIZATION 
        x = self.dropout(x)# SECOND PASS THE OUTPUT OF THE CNN THROUGH LSTM 
        
        x = x.reshape(batch_size,-1)
        #print('input of fc SIGNAL layer shape',x.shape)
        ...
        # THIRD PASS THE REFERENCE SIGNAL THROUGH THE SAME CNN LAYERS
        ...
        #print(xref.size(1))
        #print(xref.size(2))
        #xref = xref.reshape(1,-1) #facer promedio das señales do batch poñendo 1 na primeira dim
        #xref = xref.reshape(1, xref.size(1)*xref.size(2))
        xref = torch.mean(xref,0, True)
        ...
        xref = xref.repeat(batch_size,1)
        
        #print('input fc xref', xref.shape)
        #print('x', x.shape)
        #print('f',f.shape)
        z = torch.cat((x,f,xref), 1)
            ...
            with torch.no_grad(): 
                ...
                if len(self.valid_loss_during_training)>1:
                    if self.valid_loss_during_training[-1]<np.min(self.valid_loss_during_training[:-1]):
                        torch.save(self.state_dict(), path + 'checkpoint_EarlyStopping_bestvalepoch_SERVER_cnn_xref_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.pth')
...
if args.train:
    print("Training...")
    my_CNN.trainloop(trainloader, validloader, xrefloader)
    torch.save(my_CNN.state_dict(), path + 'checkpoint_LASTEPOCH_SERVER_static_cnn_6_layers_xreference_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.pth')

    # my_CNN.load_state_dict(torch.load(path + 'checkpoint_EarlyStopping_bestvalepoch_SERVER_cnn_xref_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.pth'))

    print("Evaluating...")
    # EVALUATION METRICS FOR WS1 COMPLETE DATASET
    print("Accuracies...")
    # accuracy
    acc_train = my_CNN.eval_performance(trainloader, xrefloader)
    acc_val = my_CNN.eval_performance(validloader, xrefloader)
    acc_tst = my_CNN.eval_performance(testloader, xrefloader)
    print("Ploting losses..")
    # loss
    loss_tr = my_CNN.loss_during_training
    loss_valid = my_CNN.valid_loss_during_training
    loss_curve(loss_tr, loss_valid, name= 'SERVER_loss_cnn_6lay_xref_subsam_MULTICLASS_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs), path=path)
    print("AUCs..")
    # AUC 
    auc_train = my_CNN.get_auc(trainloader, xrefloader, y_tr, name='ROC_CNN_TRAIN')
    auc_val = my_CNN.get_auc(validloader, xrefloader, y_val, name='ROC_CNN_VALID')
    auc_test = my_CNN.get_auc(testloader, xrefloader, y_test, name='ROC_CNN_TEST')
    print("AUC train: "+ str(auc_train))
    print("AUC val: "+ str(auc_val))
    print("AUC test: "+ str(auc_test))

    # save accuracy and auc of results in txt
    f = open(path + 'results_complete_multiclasss_cnn_6lay_xref_subsam_ws1_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.txt', 'w')
    ...
    print("Results by configurations...")
    ...
    for conf in grid:
        ...
        auc_ws1 = model.get_auc(dataloader_ws1, xrefloader_ws1, y_bin_ws1, name= 'ROC_CNN_WS1_'+str(list(conf.items())))
        ...
        auc_ws2 = model.get_auc(dataloader_ws2, xrefloader_ws2, y_bin_ws2, name= 'ROC_CNN_WS2_'+str(list(conf.items())) )
        ...
        auc_ws3 = model.get_auc(dataloader_ws3, xrefloader_ws3, y_bin_ws3, name= 'ROC_CNN_WS3_'+str(list(conf.items())))
        ...
        auc_ws4 = model.get_auc(dataloader_ws4, xrefloader_ws4, y_bin_ws4, name= 'ROC_CNN_WS4_'+str(list(conf.items())))
    ...
    results.to_csv(path + 'results_SERVER_CNN_COMPLETE_MODEL_MULTICLASS_xref_subsampled_lr'+str(args.lr)+'_dropprob'+str(args.drop_prob)+'_epochs'+str(args.epochs)+'.csv')
